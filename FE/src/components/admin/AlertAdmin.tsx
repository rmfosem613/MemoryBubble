...
const AlertAdmin = ({ user, font }: FontProps) => {
  return (
    <>
      <div className={`p-4 flex items-center justify-between border border-gray-700 rounded-lg bg-white w-[500px] h-[60px]`}>
        <div className="flex items-center space-x-2">
          ...
          <span className={`font-medium text-sm text-gray-700`}><span className="font-p-800 text-black">{user}</span>님이 <span className="font-p-800 text-blue-700">{font}</span> 폰트 생성을 의뢰하셨습니다.</span>
        </div>
        {/* x 누르면 완료 처리 */}
