...
import { Mail, LogOut, Settings, Copy, RotateCcw, Edit2 } from 'lucide-react';
...
const Header = () => {
  // 스토어 관련 상태
  const { user, family, isUnread, setIsUnread, resetUser } = useUserStore();
  const { checkUnreadLetter, logout, getFamilyInviteCode } = useUserApi();
            ...
            <>
              ...
              {user.familyId && user.birth ? (
                <div className="flex items-center space-x-3">
                  ...
                  <Link
                    to="/storage"
                    className="relative p-1 rounded-full hover:bg-black/10 transition-colors group">
                    <Mail
                    ...
                    {isUnread && (
                      <>
                        <span className="absolute top-[4px] right-[2px] h-2.5 w-2.5 bg-red-500 rounded-full"></span>
                        <span className="invisible group-hover:visible absolute top-full left-1/2 transform -translate-x-1/2 mt-2 px-2 py-1 bg-gray-800 text-white text-xs rounded whitespace-nowrap">
                          읽지 않은 편지가 있습니다
                        </span>
                      </>
                    )}
                ...
                // 로그인은 되었지만 가족이나 프로필이 없는 경우 로그아웃만 표시
                <button
                  className="p-2 flex items-center space-x-1 text-sm  hover:text-red-700 transition-colors"
                  onClick={handleLogout}>
                  <LogOut size={18} />
                  <p>로그아웃</p>
                </button>
