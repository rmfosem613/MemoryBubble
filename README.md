...
1. 나만의 손글씨 : Few-shot Font Generation 모델 AI를 활용해 나의 손글씨를 폰트로 생성
2. 추억 갤러리 : 추억이 담긴 사진을 업로드 할 수 있는 공간
    - 사진 뒷면 엽서에 텍스트나 음성으로 감상평 작성
    - 텍스트의 경우 만들어진 폰트가 있다면 폰트 적용
3. 편지: 텍스트나 음성으로 편지 작성
    - 느린 편지로 개봉 날짜 지정 가능
    - 텍스트의 경우 만들어진 폰트가 있다면 폰트 적용
4. 방울 캘린더 : 가족 일정 등록 가능, 앨범과 연결
   <br />
...
| 구현진 |      Infra       | - DB 설계 및 API 명세서 작성<br />- 폰트 기능<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 폰트 관련 API 개발<br />  &nbsp;&nbsp;&nbsp;&nbsp;- S3 Presigned URL을 통한 템플릿 업로드 기능 구현<br />  &nbsp;&nbsp;&nbsp;&nbsp;- CloudFront Signed URL을 통한 TTF 파일 조회 기능 구현<br />- Blue-Green 무중단 배포 환경 구성<br />  &nbsp;&nbsp;&nbsp;&nbsp;- Jenkins 파이프라인을 통한 배포 자동화<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 신규 컨테이너에 대한 Health Check로 정상 구동 여부 확인<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 빌드 실패 시, 기존 이미지를 활용한 자동 롤백 처리<br />- CloudFront와 Lambda@Edge를 활용한 이미지 리사이징<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 서버리스 기반 Lambda@Edge 함수로 실시간 이미지 리사이징 처리<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 쿼리 파라미터로 이미지 크기를 지정해 로딩 속도 및 용량 최적화<br />- 이미지 다운샘플링<br />  &nbsp;&nbsp;&nbsp;&nbsp;- 이미지 크롭 및 미리보기 시, 원본 이미지가 아닌 손실 압축된 버전을 제공해 로딩 속도 및 사용자 경험 향상<br />                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
